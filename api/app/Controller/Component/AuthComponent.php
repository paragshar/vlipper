 ...
 */
	public function startup(Controller $controller) {
		...
		if(!empty($headers['api_key'])){
			...
			$client_vendor_id = $appController->validate_client_api_key($api_key);
			if(!$client_vendor_id){
				throw new DataValidationException('Invalid api key');
			}
		}else{
			throw new DataValidationException('No api key found');
			return false;
		}*/
		...
		if ($loginAction == $url) {
			...
			if (!$this->_getUser()) {
				if (!$request->is('ajax')) {
					...
					throw new UnauthorizedAccessAttemptException('Please login to access the location');
					return false;
 ...
 */
	public function isAuthorized($user = null, $request = null) {
		CakeLog::write(LOG_DEBUG,  'i am here');
